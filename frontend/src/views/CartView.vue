<template>
    <div class="cart-view bg-dark text-white vh-100">
        <h1 class="text-center bg-dark">Your Shopping Cart</h1>
        <!-- Display spinner when loading data -->
        <spinner-loader :loading="loading" />
        <div v-if="!cartItems || cartItems.length === 0">
            <p v-if="!loading" class="vh-100 text-white text-center bg-dark">Your cart is empty.</p>
        </div>
        <div v-else>
            <table class="table table-dark table-striped container">
                <thead>
                    <tr>
                        ...
                        <!-- Conditionally render 'Amount' column based on screen width -->
                        <th scope="col" v-if="screenWidth >= 500">Amount</th>
                        <th scope="col">Quantity</th>
                        <!-- Conditionally render 'Amount' column header based on screen width -->
                        <th scope="col" v-if="screenWidth >= 500">Amount</th>
                ...
                <tbody>
                    <tr v-for="(item, index) in cartItems" :key="index">
                        ...
                        <td>{{ item.itemName }}</td>
                        <!-- Conditionally render 'Amount' column based on screen width -->
                        <td v-if="screenWidth >= 500">R {{ item.itemAmount }}</td>
                        <td>R {{ item.itemQuantity }}</td>
                        <!-- Conditionally render 'Amount' column based on screen width -->
                        <td v-if="screenWidth >= 500">R {{ item.itemAmount }}</td>
                        <td>
                            <button @click="updateCartItem(item)" class="btn btn-primary"><i
                                    class="bi bi-pencil"></i></button>
                            <button @click="removeCartItemByItemID(item.itemID)" class="btn btn-danger"><i
                                    class="bi bi-trash"></i></button>
                        </td>
                    </tr>
                </tbody>
            </table>
            <button @click="removeCartItemByCartID(item.cartID)" class="btn btn-danger"><i class="bi bi-trash"></i>Clear
                Cart</button>
        </div>
    </div>
</template>
